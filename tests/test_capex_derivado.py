...
Quem decidiu foi o motor, e antes desta suíte: `otimizadorcapex_v62.py:1165`
...
`capex_e_derivado` (`migracoes/005capex_derivado.sql`) a prende no banco, para
...
from app.dominio.ficha import capex


def testcapex_e_quantidade_vezes_preco():
    assert capex({"qtd": "100", "preco": "2.000,00"}) == 200000


def test_pt_br_com_decimal_e_a_forma_que_chega_da_tela():
    """`69,64 × 2.941,79` — a ficha `b1b25_1_1`, que motivou o item do plano."""
    assert capex({"qtd": "69,64", "preco": "2.941,79"}) == pytest.approx(204866.2556)
...
def test_fator_ausente_e_nulo_e_nao_zero():
    ...
    (`pendencias.py:OBRA`) e trava a simulação da unidade inteira. A régua é
    aquela — aqui só não se inventa o número.
    """
    assert capex({"qtd": None, "preco": "2.000,00"}) is None
    assert capex({"qtd": "100", "preco": None}) is None
    assert capex({}) is None
    assert capex({"qtd": "", "preco": "2.000,00"}) is None  # branco é ausência


def test_zero_de_verdade_continua_zero():
    """Quantidade zero é resposta, não silêncio: a obra existe e não tem extensão."""
    assert capex({"qtd": "0", "preco": "1.200,00"}) == 0
...
def test_numero_torto_e_422_e_nao_500():
    ...
    with pytest.raises(ValorInvalido) as e:
        capex({"qtd": "cem", "preco": "2.000,00"})
    assert "obra.qtd" in str(e.value)

    with pytest.raises(ValorInvalido) as e:
        capex({"qtd": "100", "preco": "dois mil"})
    assert "obra.preco" in str(e.value)


def test_o_corpo_nao_opina_sobre_ocapex():
    ...
    a simulação não usaria. `OBRA` não mapeia o campo, e o cálculo só olha os
    dois fatores.
    """
    assert capex({"qtd": "2", "preco": "3", "capex": "999.999,00"}) == 6
...
def test_a_tolerancia_da_constraint_cobre_o_arredondamento_da_planilha():
    ...
    """
    derivado = capex({"qtd": "532,05", "preco": "2.227,10"})
